...
(async function () {
  ...
  while (true) {
    const { allowance } = await (
      await fetch(
        `http://docker.for.mac.host.internal:3000/compound/allowance`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({
    ...
    let {
      ...
      await fetch(`http://docker.for.mac.host.internal:3000/compound/approve`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          investment: investment,
    ...
    const operation = await (
      await fetch(
        `${COSSET_API_URL}/wallets/${account.address.wallet.id}/sign`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({
  ...
  const {
    ...
    await fetch(`http://docker.for.mac.host.internal:3000/compound/deposit`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        investment: investment,
