...
const LUNA_ACCOUNT = 42814;

const accountID = LUNA_ACCOUNT;
...
(async function () {
  ...
  console.log('\n📄 Create new send');
  const res = await (
    await fetch(`${COSSET_CRYPTO_URL}/terra/transaction`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        wallet: {
        ...
        transaction: {
          ...
          transfers: [
            {
              address: {
                hash: 'terra1ltpkqs4eflmltpnq0rl6zx8wg5hh0ffvqk74x4',
              },
              direction: 'in',
              amount: '1000',
              currency: {
  ...
  const signedTx = await (
    await fetch(`${COSSET_CRYPTO_URL}/terra/signature`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        raw,
  ...
  const decodedTx = await (
    await fetch(`${COSSET_CRYPTO_URL}/terra/decode`, {
      method: 'POST',
      headers,
      body: JSON.stringify({
        raw: signedTx.raw,
  ...
  console.log(
    `\n*️⃣  Broadcasting: https://finder.terra.money/tequila-0004/tx/${decodedTx.hash}`,
  );

  const broadcastResponse = await fetch(
    `${COSSET_CRYPTO_URL}/terra/broadcast`,
    {
      method: 'POST',
      headers,
      body: JSON.stringify({
